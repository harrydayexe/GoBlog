...
func TestPostList_FilterPublished(t *testing.T) {
	...
	posts := PostList{
		...
		{Title: "Post 2", Date: now, Description: "desc2", Draft: true},
		{Title: "Post 3", Date: now, Description: "desc3", Draft: false},
		{Title: "Post 4", Date: now, Description: "desc4", Draft: true},
	}

	published := posts.FilterPublished()
	...
	for _, post := range published {
		if post.Draft {
			t.Errorf("filtered list should not contain draft posts, found %q", post.Title)
		}
	}
}
